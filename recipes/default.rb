...
#
file_name = "erlang-#{node['erlang']['major_version']}.#{node['erlang']['minor_version']}"
...
when 'rhel'
  ...
  package 'erlang-erts' do
    action :purge
    not_if "erl -eval 'erlang:display(erlang:system_info(otp_release)), halt().'  -noshell | grep #{node['erlang']['major_version']}"
  end
